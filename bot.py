import osimport csvimport hmacimport jsonimport timeimport reimport base64import hashlibimport sqlite3import requestsimport tempfileimport random

from urllib.parse import urlencodefrom datetime import datetime, timezone, timedeltafrom zoneinfo import ZoneInfo

from telegram import (Update,InlineKeyboardButton,InlineKeyboardMarkup,)from telegram.constants import ParseModefrom telegram.helpers import mention_htmlfrom telegram.error import TelegramErrorfrom telegram.ext import (Application,CommandHandler,CallbackQueryHandler,ChatJoinRequestHandler,MessageHandler,ContextTypes,filters,Defaults,)
...
BOT_TOKEN = os.environ["BOT_TOKEN"]VIP_CHAT_ID = int(os.environ["VIP_CHAT_ID"])
...
OKX_API_KEY = os.environ["OKX_API_KEY"]OKX_API_SECRET = os.environ["OKX_API_SECRET"]OKX_API_PASSPHRASE = os.environ["OKX_API_PASSPHRASE"]

BYPASS_CODE = os.environ.get("BYPASS_CODE", "00000000010101010")ADMIN_IDS = [int(x.strip())for x in os.environ.get("ADMIN_IDS", "").split(",")if x.strip().isdigit()]
...
TZ_AR = ZoneInfo("America/Argentina/Buenos_Aires")GROUP_NAME = "Comunidad Flanders y Fred VIP by OKX"OKX_BASE_URL = "https://www.okx.com"INSTRUMENTS_CACHE = {}

OKX_JOIN_LINK = "https://www.okx.com/join/FLANDERSYFRED"REBIND_FORM_LINK = "https://www.okx.com/ul/J6l2R5"FLANDERS_PRIVATE_LINK = "https://t.me/ivandp93"

PUBLIC_BOT_REPLY_LINK = "https://t.me/+1hKD3O8rj8ZiOTQx"

VALID_REF_CODES_TEXT = ("71790605\n""27221066\n""FLANDERSYFRED\n""ELTRADERROLO")
...
def money(value):try:return f"{float(value or 0):,.2f}"except Exception:return "0.00"

def number(value):try:return f"{float(value or 0):,.0f}"except Exception:return "0"

def safe_float(value):try:return float(value or 0)except Exception:return 0.0

def split_uids(raw_text: str):raw_text = raw_text.replace(",", " ").replace("\n", " ").replace(";", " ")return [x.strip() for x in raw_text.split() if x.strip().isnumeric()]

def ts_to_human(value):if value in [None, ""]:return ""
...
def format_optional_usdt(value):if value is None or value == "":return "No disponible"return f"{money(value)} USDT"

def contains_bot_keyword(text: str) -> bool:"""Detecta la palabra bot o bots como palabra independiente.Ejemplos válidos:- bot- bots- quiero un bot- info de bots"""if not text:return False
...
def db():db_dir = os.path.dirname(DB_PATH)if db_dir:os.makedirs(db_dir, exist_ok=True)
...
def save_user_state(telegram_id,uid=None,first_name=None,username=None,flow=None,status="pending",source=""):conn = db()cur = conn.cursor()
...
def get_user_state(telegram_id):conn = db()cur = conn.cursor()
...
def get_all_user_states():conn = db()cur = conn.cursor()
...
def get_user_state_counts():conn = db()cur = conn.cursor()
...
def save_user(telegram_id, uid, first_name=None, username=None, last_vol_month=0):conn = db()cur = conn.cursor()
...
def update_user_volume_by_uid(uid, volume):conn = db()cur = conn.cursor()
...
def get_user_by_telegram_id(telegram_id):conn = db()cur = conn.cursor()
...
def get_user_by_uid(uid):conn = db()cur = conn.cursor()
...
def get_all_users():conn = db()cur = conn.cursor()
...
def get_okx_server_time_iso():r = requests.get(f"{OKX_BASE_URL}/api/v5/public/time", timeout=10)r.raise_for_status()
...
def sign_okx(method, path, body=""):timestamp = get_okx_server_time_iso()message = timestamp + method + path + body
...
def okx_get(path):ts, signature = sign_okx("GET", path)
...
def okx_public_get(path):url = OKX_BASE_URL + pathreturn requests.get(url, timeout=20).json()
...
def okx_affiliate_detail(uid):path = f"/api/v5/affiliate/invitee/detail?uid={uid}"return okx_get(path)

def is_uid_affiliated(uid):resp = okx_affiliate_detail(uid)
...
def parse_okx_invitee_detail(resp):if resp.get("code") != "0" or not resp.get("data"):return None
...
def get_uid_volume(uid):resp = okx_affiliate_detail(uid)parsed = parse_okx_invitee_detail(resp)
...
def get_uid_report(uid):resp = okx_affiliate_detail(uid)parsed = parse_okx_invitee_detail(resp)
...
def okx_trade_fills_history(inst_type, begin_ms, end_ms, after=None, limit=100):params = {"instType": inst_type,"begin": str(begin_ms),"end": str(end_ms),"limit": str(limit)}
...
def get_instrument_info(inst_type, inst_id):cache_key = f"{inst_type}:{inst_id}"
...
def estimate_fill_volume_usdt(fill):inst_type = fill.get("instType", "")inst_id = fill.get("instId", "")
...
def parse_period_to_days(args):if not args:return 7
...
def get_master_trading_volume(days=7):end_dt = datetime.now(timezone.utc)begin_dt = end_dt - timedelta(days=days)
...
def format_master_volume_report(report):errors_text = ""
...
def format_uid_report(report):if report is None:return "❌ UID no encontrado en la comunidad de afiliados OKX."
...
def format_uid_report_line(report, uid):if report is None:return f"❌ {uid} | No afiliado / no encontrado"
...
def start_keyboard():return InlineKeyboardMarkup([[InlineKeyboardButton("🆕 Soy nuevo", callback_data="flow_new")],[InlineKeyboardButton("👤 Ya tengo cuenta OKX", callback_data="flow_existing")],[InlineKeyboardButton("🔄 Ya estoy en VIP y debo actualizar referido", callback_data="flow_update")],])

def action_keyboard():return InlineKeyboardMarkup([[InlineKeyboardButton("✍️ Enviar mi UID de OKX", callback_data="ask_uid")],[InlineKeyboardButton("🔙 Ver menú principal", callback_data="show_menu")],])

def back_to_menu_keyboard():return InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Ver menú principal", callback_data="show_menu")],])

def uid_prompt_text():return ("✍️ Perfecto.\n\n""Ahora envíame tu UID de OKX usando solo números.\n\n""Ejemplo:\n""123456789")

def welcome_menu_text():return (f"👋 Hola, soy el asistente oficial de {GROUP_NAME}.\n\n""Te voy a guiar para acceder o mantener tu acceso a los bots y copy trading de la comunidad.\n\n""Elige una opción para continuar:")

def terms_text():return ("📌 Términos y condiciones importantes\n\n""✅ La inversión mínima depende del bot que elijas, pero ronda en 25 USDT. ""Actualmente hay bots sobre diferentes activos como XAU, ETH, BTC, TSLA, QQQ, XAG, XRP, SPY, petróleo, ""acciones tecnológicas y otros instrumentos disponibles.\n\n""✅ Existen algunos bots exclusivos con mínimos de inversión mayores a 1000 USDT. ""Más información en el grupo VIP.\n\n""✅ La inversión mínima para copy trading depende de cada trader o estrategia. ""En general, puede comenzar desde aproximadamente 100 USDT.\n\n""✅ El servicio de bots y copy trading es gratuito. Los bots no están a la venta y no hay membresías. ""Funcionan únicamente en OKX. No operan en brokers externos ni en MT5.\n\n""✅ Tú tienes el control total de tu cuenta de OKX. Puedes seguir o dejar de seguir un bot o copy trading ""cuando lo decidas. Puedes depositar más o retirar tu capital cuando quieras.\n\n""❌ No garantizamos rendimientos fijos ni seguros.\n\n""❌ Los bots no funcionan como un CDT, depósito a plazo o producto de renta fija.\n\n""❌ No somos asesores financieros. Cada usuario debe evaluar por su cuenta qué bot, copy trading ""o estrategia desea seguir.\n\n""❌ No captamos dinero del público ni gestionamos recursos de terceros.\n\n""❌ No somos una entidad financiera, casa de valores, sociedad comisionista de bolsa ""ni administradores de portafolios.")

def new_user_text():return ("🚀 Para NUEVOS usuarios\n\n""1️⃣ Crea tu cuenta en OKX con el link oficial:\n\n"f"{OKX_JOIN_LINK}\n\n""2️⃣ Completa tu registro y verificación KYC.\n\n""3️⃣ Solicita acceso al grupo VIP.\n\n""4️⃣ Cuando el bot te escriba, envía tu UID de OKX por este chat.\n\n""5️⃣ Si tu UID está correctamente vinculado, el bot aprobará tu acceso automáticamente al grupo VIP.\n\n""Dentro del grupo VIP encontrarás:\n\n""🤖 Links de bots disponibles\n""📈 Copy trading\n""🎁 Bonos y beneficios especiales\n""📚 Información y soporte para operar en OKX\n\n""🚀 Así de simple: crea tu cuenta, solicita acceso, valida tu UID y entra a la comunidad VIP para comenzar a participar.")

def existing_user_text():return ("👤 Si ya tienes cuenta en OKX\n\n""Debes comprobar que tu cuenta esté vinculada a alguno de estos códigos/referidos:\n\n"f"{VALID_REF_CODES_TEXT}\n\n""✅ ¿Cómo saber si estás vinculado correctamente?\n\n""Envíame tu UID de OKX por este chat.\n\n""Si el sistema valida tu UID, el bot podrá aprobar tu acceso al grupo VIP.\n\n""Si no puedes avanzar, significa que tu cuenta no está vinculada correctamente al referido.\n\n""Si ya tienes cuenta, pero no estás vinculado al código de referido, completa el siguiente formulario:\n\n"f"{REBIND_FORM_LINK}\n\n""En el campo de código de invitación, ingresa:\n\n""FLANDERSYFRED\n""o el código:\n""99142589\n\n""En motivo, escribe:\n\n""Quiero pertenecer a la comunidad de FLANDERS Y FRED\n\n""Si te aparece un mensaje indicando que por el momento no puedes continuar, escribe al privado de Flanders ""y envía tu UID para revisar tu caso:\n\n"f"{FLANDERS_PRIVATE_LINK}")

def update_ref_text():return ("🔄 Actualización de referido para usuarios que ya están en el grupo VIP\n\n""Debes comprobar que tu cuenta esté vinculada a alguno de estos códigos/referidos. ""De lo contrario, podrás ser removido del grupo el día 10 de julio y perderás el acceso ""a los nuevos bots y copy trading exclusivos.\n\n"f"{VALID_REF_CODES_TEXT}\n\n""✅ ¿Cómo saber si estás vinculado correctamente?\n\n""Envíame tu UID de OKX por este chat.\n\n""Si el sistema valida tu UID, podrás seguir en el grupo VIP.\n\n""Si no puedes avanzar, significa que tu cuenta no está vinculada correctamente al referido.\n\n""Para cambiar el código de referido, completa el siguiente formulario:\n\n"f"{REBIND_FORM_LINK}\n\n""En el campo de código de invitación, ingresa:\n\n""FLANDERSYFRED\n""o el código:\n""99142589\n\n""En motivo, escribe:\n\n""Quiero pertenecer a la comunidad de FLANDERS Y FRED\n\n""Si te aparece un mensaje indicando que por el momento no puedes continuar, escribe al privado de Flanders ""y envía tu UID para revisar tu caso:\n\n"f"{FLANDERS_PRIVATE_LINK}")

def not_affiliated_existing_text():return ("❌ Tu UID no aparece vinculado a la comunidad de Flanders y Fred.\n\n""Debes comprobar que tu cuenta esté vinculada a alguno de estos códigos/referidos:\n\n"f"{VALID_REF_CODES_TEXT}\n\n""Si ya tienes cuenta, pero no estás vinculado al código de referido, completa el siguiente formulario:\n\n"f"{REBIND_FORM_LINK}\n\n""En el campo de código de invitación, ingresa:\n\n""FLANDERSYFRED\n""o el código:\n""99142589\n\n""En motivo, escribe:\n\n""Quiero pertenecer a la comunidad de FLANDERS Y FRED\n\n""Si te aparece un mensaje indicando que por el momento no puedes continuar, escribe al privado de Flanders ""y envía tu UID para revisar tu caso:\n\n"f"{FLANDERS_PRIVATE_LINK}")

def not_affiliated_update_text():return ("⚠️ Tu UID no aparece vinculado a la comunidad de Flanders y Fred.\n\n""Tienes hasta el 10 de julio para mantener tus accesos a los nuevos bots y copy trading exclusivos.\n\n""Debes comprobar que tu cuenta esté vinculada a alguno de estos códigos/referidos:\n\n"f"{VALID_REF_CODES_TEXT}\n\n""Para cambiar el código de referido, completa el siguiente formulario:\n\n"f"{REBIND_FORM_LINK}\n\n""En el campo de código de invitación, ingresa:\n\n""FLANDERSYFRED\n""o el código:\n""99142589\n\n""En motivo, escribe:\n\n""Quiero pertenecer a la comunidad de FLANDERS Y FRED\n\n""Si te aparece un mensaje indicando que por el momento no puedes continuar, escribe al privado de Flanders ""y envía tu UID para revisar tu caso:\n\n"f"{FLANDERS_PRIVATE_LINK}")

def validated_text(flow):if flow == "update":return ("✅ UID verificado correctamente.\n\n""Gracias por actualizar tu información. Tu cuenta aparece vinculada a la comunidad de Flanders y Fred.\n\n""Puedes acceder de forma permanente a los bots y copy trading de Flanders y Fred mientras mantengas ""las condiciones de la comunidad.")
...
def group_welcome_text(user):return (f"🚀👋 Bienvenido {mention_html(user.id, user.first_name)} al grupo {GROUP_NAME}.\n\n""Aquí encontrarás bots exclusivos, copy trading, tips de trading y beneficios por pertenecer a nuestra comunidad.\n\n"f"{terms_text()}\n\n""¡Saludos y buenos trades! 📈")

def public_bot_reply_text():return ("Hola, ¿cómo estás? Para seguir los Bots y los copy tradings, sigue las instrucciones del siguiente link.\n\n""Si el chat no inicia automáticamente, escribe Hola.\n\n"f"{PUBLIC_BOT_REPLY_LINK}")

async def send_welcome(context, user):await context.bot.send_message(chat_id=VIP_CHAT_ID,text=group_welcome_text(user),parse_mode=ParseMode.HTML)

async def approve_user_if_possible(context: ContextTypes.DEFAULT_TYPE, user):try:await context.bot.approve_chat_join_request(VIP_CHAT_ID, user.id)print(f"✅ Solicitud aprobada TG={user.id}")return Trueexcept TelegramError as e:print(f"⚠️ No se pudo aprobar solicitud TG={user.id}: {e}")return Falseexcept Exception as e:print(f"⚠️ Error aprobando solicitud TG={user.id}: {e}")return False
...
async def menu(update: Update, context: ContextTypes.DEFAULT_TYPE):await start(update, context)

async def flow_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):query = update.callback_queryawait query.answer()
...
async def on_join_request(update: Update, context: ContextTypes.DEFAULT_TYPE):user = update.chat_join_request.from_user
...
async def track_new_group_members(update: Update, context: ContextTypes.DEFAULT_TYPE):if update.effective_chat.id != VIP_CHAT_ID:return
...
async def public_bot_keyword_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):"""Responde automáticamente en el grupo público cuando alguien escribe la palabra:bot o bots."""if not update.message or not update.message.text:return
...
async def handle_private(update: Update, context: ContextTypes.DEFAULT_TYPE):user = update.message.from_usertext = update.message.text.strip()
...
async def mivolumen(update: Update, context: ContextTypes.DEFAULT_TYPE):if update.effective_chat.type != "private":await update.message.reply_text("Por seguridad, usa este comando en el chat privado con el bot.")return
...
async def lista(update: Update, context: ContextTypes.DEFAULT_TYPE):if update.effective_chat.type != "private":await update.message.reply_text("Por seguridad, usa este comando por privado.")return
...
async def listauids(update: Update, context: ContextTypes.DEFAULT_TYPE):if update.effective_chat.type != "private":await update.message.reply_text("Por seguridad, usa /listauids por privado.")return
...
async def sorteo(update: Update, context: ContextTypes.DEFAULT_TYPE):if update.effective_chat.type != "private":await update.message.reply_text("Por seguridad, usa este comando por privado.")return
...
async def top(update: Update, context: ContextTypes.DEFAULT_TYPE):if update.effective_chat.type != "private":await update.message.reply_text("Por seguridad, usa este comando por privado.")return
...
async def voluid(update: Update, context: ContextTypes.DEFAULT_TYPE):if update.effective_chat.type != "private":await update.message.reply_text("Por seguridad, usa este comando en el chat privado con el bot.")return
...
async def checkuid(update: Update, context: ContextTypes.DEFAULT_TYPE):if update.effective_chat.type != "private":await update.message.reply_text("Por seguridad, usa este comando por privado.")return
...
async def checkuids(update: Update, context: ContextTypes.DEFAULT_TYPE):if update.effective_chat.type != "private":await update.message.reply_text("Por seguridad, usa este comando por privado.")return
...
async def checkuidscsv(update: Update, context: ContextTypes.DEFAULT_TYPE):if update.effective_chat.type != "private":await update.message.reply_text("Por seguridad, usa este comando por privado.")return
...
async def debuguid(update: Update, context: ContextTypes.DEFAULT_TYPE):if update.effective_chat.type != "private":await update.message.reply_text("Por seguridad, usa este comando por privado.")return
...
async def informe(update: Update, context: ContextTypes.DEFAULT_TYPE):if update.effective_chat.type != "private":await update.message.reply_text("Por seguridad, usa /informe en el chat privado con el bot.")return
...
async def weekly_admin_report(context: ContextTypes.DEFAULT_TYPE):await generate_admin_report(context, "📊 REPORTE SEMANAL ADMIN FLANDERS Y FRED")

async def monthly_admin_report(context: ContextTypes.DEFAULT_TYPE):now = datetime.now(TZ_AR)
...
async def generate_admin_report(context, title):total_volumen = 0.0usuarios = get_all_users()
